 ...
 */
public abstract class DaoImpl<T, PK extends Serializable> extends HibernateDaoSupport implements Dao<T, PK> {
        ...
        if (values != null) {
            for (int i = 0; i < values.length; i++) {
                ...
                if(values[i] instanceof Collection<?>){
                    query.setParameterList("arr_"+i, (Collection) values[i]);
                }else if(values[i] instanceof Object[]){
                    query.setParameterList("arr_"+i, (Object[]) values[i]);
                }else{
                    query.setParameter(i, values[i]);
                }
